...
/**
 * Clase para construir y gestionar un árbol de decisiones.
 */
public class DecisionTree {
    ...
    private DecisionTreeNode buildTree(int questionIndex, List<Integer> currentIndices) {
        if (currentIndices.isEmpty()) {
            return null; // No hay respuestas que procesar.
        }

        if (questionIndex >= questions.size()) {
            // Todas las respuestas en currentIndices son del mismo animal si no quedan preguntas.
        ...
        DecisionTreeNode currentNode = new DecisionTreeNode(currentQuestion);
        ...
        } else if (yesBranchIndices.isEmpty() && noBranchIndices.isEmpty()) {
            // Si no hay elementos en ambas ramas, asumir que todas las respuestas llevan al mismo animal
            String animal = answers.get(currentIndices.get(0))[0]; // Tomar el nombre del animal del primer índice.
            return new DecisionTreeNode(animal);
        } else {
            currentNode.yesBranch = new DecisionTreeNode("No se encontró un animal con estas características.");
        }
        ...
        if (!noBranchIndices.isEmpty()) {
            ...
            currentNode.noBranch = new DecisionTreeNode("No se encontró un animal con estas características.");
        }
